...
class Account < ApplicationRecord
  ...
  validates :username, format: { with: /\A[a-z0-9_]+\z/i }, length: { maximum: 60 }, if: -> { local? && will_save_change_to_username? && actor_type != 'Application' }
  validates_with UnreservedUsernameValidator, if: -> { local? && will_save_change_to_username? && actor_type != 'Application' }
  validates :display_name, length: { maximum: 60 }, if: -> { local? && will_save_change_to_display_name? }
  validates :note, note_length: { maximum: 1500 }, if: -> { local? && will_save_change_to_note? }
